...
/*    //For Camera shake 
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.C))
        {
            ...
            string prefabPath = Path.Combine("Prefabs", "Projectiles(transform)", Prefabs[Prefab].name);
            PhotonNetwork.Instantiate(prefabPath, FirePoint.transform.position, FirePoint.transform.rotation);
        }

        //Fast shooting
        if (Input.GetKeyDown(KeyCode.V) && fireCountdown <= 0f)
        {
            string prefabPath = Path.Combine("Prefabs", "Projectiles(transform)", Prefabs[Prefab].name);
            PhotonNetwork.Instantiate(prefabPath, FirePoint.transform.position, FirePoint.transform.rotation);
            fireCountdown = 0;
